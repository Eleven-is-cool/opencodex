...
export const ja: Record<TKey, string> = {
  ...
  "dash.mem.hint": "読み取り専用のランタイム診断。RSS が増加する一方で JS ヒープが平坦な場合、アプリレベルのリークではなくネイティブランタイムメモリを示します。",
  ...
  "codexAuth.autoSwitchDesc": "使用率が {threshold}% 以上で、より使用率の低い利用可能なアカウントがある場合に切り替えます。Go/Free は 30 日枠のみを使用します",
